...
import com.rickdane.farmersmarkets.util.LocationsUtil;
...
public class TabFragment extends Fragment {
    ...
    //to check whether we have performed the initial on-load query, which is auto-generated by user's current location
    private boolean hasPerformedInitialOnLoadQuery = false;
    private static int lastTabState;
    ...
    /**
     * What the activity calls on instantiation, necessary because of orientation change issue, where, when orientation changes, new activity is created
     * but we don't want to reset the user's view
     */
    public void displayDefault() {
        ...
        if (lastTabState == 0) {
            ...
            //app has already been in use, probably an orientation change
            if (lastTabState == LISTINGS_STATE) {
                goToSearchResultsView(lastQuery);
            } else if (lastTabState == DETAILS_STATE) {
                gotoDetailsView(lastItem);
            }
        }
    ...
    public void goToSearchResultsView(String query) {
        ...
        if (!hasPerformedInitialOnLoadQuery && query == null) {
            //attempt to get user's zip code to do auto-generated location search
            query = LocationsUtil.getZipFromCurUserLocation(getActivity());
            hasPerformedInitialOnLoadQuery = true;
        }


        if (curTabState != LISTINGS_STATE) {
            curTabState = LISTINGS_STATE;
            lastTabState = curTabState;
